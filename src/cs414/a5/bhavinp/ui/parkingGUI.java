...
import cs414.a5.bhavinp.entities.CashPayStrategy;
import cs414.a5.bhavinp.entities.CreditCard;
import cs414.a5.bhavinp.entities.CreditCardStrategy;
...
import cs414.a5.bhavinp.entities.PaymentContext;
...
public class parkingGUI extends JFrame implements Serializable {
	...
	private void closeGateActionPerformed(java.awt.event.ActionEvent evt) {
		try {
			...
			JOptionPane.showMessageDialog(null, "Gate is closed ");
			Status status = parkingManager.closeTheEntryGate(entryGateNumber);
	...
	private void jButton5ActionPerformed(java.awt.event.ActionEvent evt) throws Exception {
		...
		// JOptionPane.showMessageDialog(null, choiceSelection);
		...
		if (choiceSelection == "Hourly") {
			...
			List<Cash> reportWeeklycash = new ArrayList<Cash>();
			...
			if (isCreditpay) {
				...
				try {
...

					{
						...
						System.out.println((String.format("    %tc             %s         %s       %f   ", cc.getCreditcardpaymentTime(),
								cc.getCCNumner().toString(), cc.getTicketID().toString(), cc.getAmount())));
					}
			...
			if (isCashPay) {
				try {
					...
					for (Cash c : parkingManager.getCashReportcollection()) {

						if (hrly.format(c.getPaymentDate()).equals(stDate)) {
							totalCollection += c.getAmount();
							reportWeeklycash.add(c);
						}
					}

					for (Cash c : reportWeeklycash)

					{

						System.out.println((String.format("    %tc            %s       %f   ", c.getPaymentDate(), c.getTicketID()
								.toString(), c.getAmount())));
					}
	...
	private void ReportActionPerformed(java.awt.event.ActionEvent evt) throws ParseException, RemoteException {
		...
		if (choiceSelection == "Weekley") {
			...
			String stDate = JOptionPane.showInputDialog("Enter any Date Input: mm/dd/yy");

			Date dtweekStart = hrly.parse(stDate);

			// String stendDate =
			// JOptionPane.showInputDialog("Enter Week End Date Input: mm/dd/yy");
